...
function replaceGenerated(sourceTag,rows){if(!sourceTag)throw new Error('sourceTag required');var s=read();if(!s)throw new Error('No saved Bruno Electric job found');s.materialsUsed=Array.isArray(s.materialsUsed)?s.materialsUsed:[];s.catalog=Array.isArray(s.catalog)?s.catalog:[];
  ...
  var added=(rows||[]).map(function(r,i){var m=catalogMatch(s.catalog,r.item);var price=m?(Number(m.yourCost!=null&&m.yourCost!==''?m.yourCost:m.unitCost)||0):0;return {
    ...
    generatedBy:{source:sourceTag,version:String(r.version||'phase1'),index:i},bomStatus:r.status||(!m?'Unresolved':price===0?'Field Verify':'Required'),codeSource:r.codeSource||'',catalogMatchId:m&&m.id||null
  }});
  s.materialsUsed=keep.concat(added);save(s);return {added:added.length,preserved:keep.length,unresolved:added.filter(function(x){return !x.catalogMatchId||x.unitCost===0}).length};
}
