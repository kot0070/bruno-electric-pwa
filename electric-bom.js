...
function prepareReplacement(state,sourceTag,rows){if(!sourceTag)throw new Error('sourceTag required');if(!state)throw new Error('No saved Bruno Electric job found');var s=clone(state);s.materialsUsed=Array.isArray(s.materialsUsed)?s.materialsUsed:[];s.catalog=Array.isArray(s.catalog)?s.catalog:[];
  ...
  s.materialsUsed=keep.concat(added);return{state:s,result:{added:added.length,preserved:keep.length,unresolved:added.filter(function(x){return x.bomStatus==='Unresolved'}).length}};
}
...
root.BrunoElectricBOM=Object.freeze({replaceGenerated:replaceGenerated,prepareReplacement:prepareReplacement,readJob:read,_match:catalogMatch,_storageKey:STORAGE});
})(window);
