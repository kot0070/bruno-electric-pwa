...
var VERSION='electrical-task-archive-v1',JOB_KEY='bruno-electric-v1';
...
function recalculate(id){var t=API.get(id);if(!t)throw new Error('Electrical Task not found in active Job');if(t.taskType!=='FEEDER_PANEL_RUN')throw new Error('Recalculate is not supported for this task type yet');var g=G.calculate(t.inputs||{},groundingFromInputs(t.inputs));t.status=g.status;t.result=clone(g.result);t.candidates=clone(g.candidates||[]);t.calculationSteps=clone(g.calculationSteps||[]);t.warnings=clone(g.warnings||[]);t.unresolved=clone(g.unresolved||[]);t.engineVersion=g.engineVersion||t.engineVersion;return API.save(t)}
