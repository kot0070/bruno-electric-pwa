...
var VERSION='feeder-panel-engine-v2';
...
function search(input){var n=normalize(input),all=[],mi=maxIndex(n),maxSets=n.parallelAllowed?6:1;for(var sets=1;sets<=maxSets;sets++){for(var i=0;i<=mi;i++){var c=candidate(n,SIZES[i],sets);if(c)all.push(c)}}var pass=all.filter(function(x){return x.passes});pass.sort(function(a,b){return a.sets-b.sets||a.totalCmil-b.totalCmil||SIZES.indexOf(a.size)-SIZES.indexOf(b.size)});pass.forEach(function(x,i){x.rank=i+1;x.selectionBasis=i===0?'FEWEST_PARALLEL_SETS_THEN_MINIMUM_TOTAL_CIRCULAR_MIL_AREA':'ALTERNATE_SUPPORTED_CONFIGURATION'});return{inputs:n,candidates:pass.slice(0,8),evaluatedCount:all.length,status:pass.length?'PASS':'NO_SUPPORTED_CONFIGURATION'};}
