/* Bruno Electric — Electrical Tasks Stage 1 job-scoped draft/persistence API. */
(function(root){
'use strict';
var JOB_KEY='bruno-electric-v1',VERSION='electrical-tasks-shell-v1',FIELD='electricalTasks',ACTIVE='electricalTaskActiveId';
var TYPES=Object.freeze([
  {id:'FEEDER_PANEL_RUN',label:'Feeder / Panel Run',enabled:true},
  {id:'BRANCH_CIRCUIT_RUN',label:'Branch Circuit Run',enabled:false},
  ...
  {id:'TRANSFORMER_FEED',label:'Transformer Feed',enabled:false},
  {id:'MOTOR_CIRCUIT',label:'Motor Circuit',enabled:false},
  {id:'EVSE_CIRCUIT',label:'EVSE Circuit',enabled:false},
  {id:'HVAC_CIRCUIT',label:'HVAC Circuit',enabled:false},
  {id:'GENERATOR_FEEDER',label:'Generator / Feeder',enabled:false},
...
function feederInputs(seed){seed=seed||{};return{
  loadAmps:raw(seed.loadAmps),
  voltage:raw(seed.voltage),
  phase:raw(seed.phase),
  distanceFt:raw(seed.distanceFt),
  material:raw(seed.material),
  installation:raw(seed.installation),
  conductorType:raw(seed.conductorType),
  loadBasis:raw(seed.loadBasis),
  vdTargetPct:raw(seed.vdTargetPct),
  ambientC:raw(seed.ambientC),
  ccc:raw(seed.ccc),
  terminalRating:raw(seed.terminalRating),
  parallelAllowed:seed.parallelAllowed===true,
  maxConductorSize:raw(seed.maxConductorSize),
  racewayStrategy:raw(seed.racewayStrategy)
}}
function make(type,seed,j){var meta=typeMeta(type);if(!meta)throw new Error('Unsupported Electrical Task type');if(!meta.enabled)throw new Error(meta.label+' is not enabled in this stage');seed=seed||{};var t=now();return{
  id:uid(),taskType:type,name:raw(seed.name)||meta.label,createdAt:t,updatedAt:t,revision:1,status:'DRAFT',
  inputs:type==='FEEDER_PANEL_RUN'?feederInputs(seed.inputs):{},
  assumptions:{},result:null,calculationSteps:[],warnings:[],unresolved:[],
  sourceEdition:currentEdition(j||{}),jurisdiction:currentJurisdiction(j||{}),engineVersion:VERSION
}}
function normalize(task,existing,j){if(!task||typeof task!=='object'||Array.isArray(task))throw new Error('Electrical Task must be an object');var meta=typeMeta(task.taskType);if(!meta||!meta.enabled)throw new Error('Unsupported or not-yet-enabled Electrical Task type');var created=existing&&existing.createdAt||task.createdAt||now();var rev=existing?((Number(existing.revision)||1)+1):(Number(task.revision)||1);return{
  id:existing&&existing.id||task.id||uid(),taskType:meta.id,name:raw(task.name).trim()||meta.label,createdAt:created,updatedAt:now(),revision:rev,status:'DRAFT',
  inputs:meta.id==='FEEDER_PANEL_RUN'?feederInputs(task.inputs):{},
  assumptions:clone(task.assumptions||{}),result:null,calculationSteps:[],warnings:clone(task.warnings||[]),unresolved:clone(task.unresolved||[]),
  sourceEdition:task.sourceEdition==null?currentEdition(j||{}):task.sourceEdition,
  jurisdiction:task.jurisdiction==null?currentJurisdiction(j||{}):task.jurisdiction,
