...
function ensureStyle(){var d=root.document;if(!d||d.getElementById('be-pricing-domain-style'))return;var s=d.createElement('style');s.id='be-pricing-domain-style';s.textContent='#be-pricing-domain-alert{position:sticky;top:0;z-index:10000;margin:0;padding:10px 14px;background:#4a1717;color:#ffd7d7;border-bottom:2px solid #ff6b6b;font:700 13px/1.35 system-ui,sans-serif}#be-pricing-domain-alert small{font-weight:500;display:block;margin-top:3px}.be-pricing-domain-blocked{opacity:.55!important;cursor:not-allowed!important}';d.head&&d.head.appendChild(s)}
function alertHost(){var d=root.document;if(!d||!d.body)return null;var x=d.getElementById('be-pricing-domain-alert');if(x)return x;x=d.createElement('div');x.id='be-pricing-domain-alert';x.hidden=true;d.body.insertBefore(x,d.body.firstChild);return x}
function controls(){if(!root.document)return[];var ids=['qa-approve','qa-print-fixed','btn-print-tm','btn-print-tm-2','btn-print-quote','btn-print-quote-2'];return ids.map(function(id){return root.document.getElementById(id)}).filter(Boolean)}
function render(){if(!root.document)return validateJob(read()||{});ensureStyle();var r=validateJob(read()||{}),h=alertHost();if(h){h.hidden=r.valid;if(r.valid)h.textContent='';else{var first=r.errors[0];h.innerHTML='PRICING / INVOICE BLOCKED — invalid numeric domain<small>'+String(first.path).replace(/</g,'&lt;')+': '+String(first.rule).replace(/</g,'&lt;')+(r.errors.length>1?' · '+(r.errors.length-1)+' additional error(s)':'')+'</small>'}}
...
if(root.document){if(root.document.readyState==='loading'&&root.document.addEventListener)root.document.addEventListener('DOMContentLoaded',install,{once:true});else install()}
