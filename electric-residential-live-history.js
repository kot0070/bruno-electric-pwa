...
function finalizeSnapshot(snapshot){var s=stripPricing(snapshot||{});s.id=s.id||id();s.savedAt=now();s.name=String(s.name||('Residential '+(s.inputs&&s.inputs.squareFeet||'')+' ft²')).trim();return s}
...
function save(snapshot){var j=readJob();if(!j)throw new Error('No saved Bruno Electric job found');var s=finalizeSnapshot(snapshot),lib=nextLibrary(library(),s),nextJob=clone(j);nextJob.residentialLiveActive=clone(s);commitPair(nextJob,lib);return hydrate(s,nextJob)}
function confirmAtomic(snapshot,sourceTag,rows){var B=root.BrunoElectricBOM;if(!B||typeof B.prepareReplacement!=='function')throw new Error('BOM transaction helper unavailable');var currentJob=readJob();if(!currentJob)throw new Error('No saved Bruno Electric job found');var prepared=B.prepareReplacement(currentJob,sourceTag,rows),s=finalizeSnapshot(snapshot),nextJob=prepared.state;nextJob.residentialLiveActive=clone(s);var lib=nextLibrary(library(),s);commitPair(nextJob,lib);return{saved:hydrate(s,nextJob),bomResult:prepared.result}}
