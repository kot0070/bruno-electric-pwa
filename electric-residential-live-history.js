 ...
 * Save Calculation is intentionally separate from Apply to Job.
 */
(function(root){
'use strict';
...
function currentPricing(snapshot,job){var P=root.BrunoResidentialPricing;if(!P||typeof P.priceRows!=='function')return null;var rows=snapshot&&Array.isArray(snapshot.bom)?snapshot.bom:[];var catalog=job&&Array.isArray(job&&job.catalog)?job.catalog:[];return P.priceRows(rows,catalog)}
...
function commitPair(nextJob,nextLib){var oldJobRaw=localStorage.getItem(JOB_KEY),oldLibRaw=localStorage.getItem(LIB_KEY),jobWritten=false,libWritten=false;try{
  ...
  var jobOk=localStorage.getItem(JOB_KEY)===oldJobRaw,libOk=localStorage.getItem(LIB_KEY)===oldLibRaw;if(!jobOk||!libOk||rollbackErrors.length)throw new Error('Save failed and rollback could not fully restore prior state. Reload before continuing. '+rollbackErrors.join('; '));throw err;
 }
...
/* Backward-compatible UI entry point. Since Stage 4, this performs SAVE ONLY.
 * sourceTag/rows are accepted for old callers but deliberately ignored.
 * Applying saved BOM to the Job is owned by BrunoResidentialApplyJob.
 */
function confirmAtomic(snapshot,sourceTag,rows){var currentJob=readJob();if(!currentJob)throw new Error('No saved Bruno Electric job found');var beforeUsed=JSON.stringify(currentJob.materialsUsed||[]),beforeUnresolved=JSON.stringify(currentJob.materialsUnresolved||[]),s=finalizeSnapshot(snapshot,currentJob.residentialLiveActive),nextJob=clone(currentJob);nextJob.residentialLiveActive=clone(s);var lib=nextLibrary(library(),s);commitPair(nextJob,lib);if(JSON.stringify(nextJob.materialsUsed||[])!==beforeUsed||JSON.stringify(nextJob.materialsUnresolved||[])!==beforeUnresolved)throw new Error('Save Calculation must not mutate Job Materials');return{saved:hydrate(s,nextJob),bomResult:{added:0,preserved:(nextJob.materialsUsed||[]).length,unresolved:0,unresolvedPreserved:(nextJob.materialsUnresolved||[]).length,saveOnly:true}}}
