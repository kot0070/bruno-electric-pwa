...
function calculate(input){input=input||{};var sqft=num(input.squareFeet,'square feet');if(sqft<=0)throw new Error('square feet must be > 0');
 ...
 var groupingGeneralCircuits=ceilDiv(generalRecs,grouping);
 var branchVAperFt2=R&&R.minimum&&Number(R.minimum.branchCircuitGeneralVAperFt2)||3;
 var branchCircuitLoadVA=sqft*branchVAperFt2;
 var branchCircuitLoadA=branchCircuitLoadVA/120;
 var codeGeneralCircuits=Math.ceil(branchCircuitLoadA/generalAmps);
 var generalCircuits=Math.max(codeGeneralCircuits,groupingGeneralCircuits);
 ...
 var circuits=[];var c=circuit('General habitable rooms',generalCircuits,generalAmps,wire,wireItem,'NEC 2026 120.13; Table 210.24(1); 210.19; 210.20','Circuit count is the greater of the §120.13 floor-area branch-circuit minimum ('+codeGeneralCircuits+') and Bruno receptacle-grouping design count ('+groupingGeneralCircuits+'). Receptacle device count is code-derived only when a complete validated wall-segment list is supplied.','CODE MINIMUM + DESIGN');if(c)circuits.push(c);
 ...
 var serviceNote='Changing ordinary receptacle quantity usually changes branch takeoff, but dwelling general branch circuits cannot fall below the separate NEC 2026 §120.13 floor-area minimum. Ordinary receptacle quantity does not directly add dwelling service-load VA under the service/feeder floor-area method. Major fixed loads must be entered in the Residential Load tool for service sizing.';
 var unresolvedLayout=!wall.known||baths>0||powder>0||kitchens>0;
 return{version:'residential-live-v3',inputs:{squareFeet:sqft,bedrooms:bedrooms,livingRooms:living,diningRooms:dining,offices:offices,bathrooms:baths,powderRooms:powder,kitchens:kitchens,laundryAreas:laundry,garageBays:garage},codeMinimums:{generalReceptacles:{known:wall.known,value:wall.known?wall.count:null,reference:wall.reference,basis:wall.basis,nonqualifyingSegments:wall.nonqualifyingSegments},generalBranchCircuits:{known:true,value:codeGeneralCircuits,loadVA:branchCircuitLoadVA,loadA:branchCircuitLoadA,ampsPerCircuit:generalAmps,reference:'NEC 2026 120.13',basis:'Dwelling branch-circuit unit load = '+branchVAperFt2+' VA/ft²; divide by 120 V and selected 15A/20A general-circuit rating, then round circuit count up.'},bathroomReceptacles:{known:false,value:null,reference:'NEC 2026 210.52(D)',basis:'Receptacle quantity/location is tied to each sink and actual placement geometry. Bathroom/powder room count alone is insufficient for an exact device minimum.'},kitchenReceptacles:{known:false,value:null,reference:'NEC 2026 210.52(C)',basis:'Countertop/work-surface receptacle quantity is layout-driven and cannot be derived from kitchen count alone.'}},design:{generalReceptacles:generalRecs,bathroomReceptacles:bathRecs,kitchenReceptacles:kitchenRecs,outdoorReceptacles:outdoorRecs,lights:lights,switches:switches,generalCircuitGroupingCount:groupingGeneralCircuits},summary:{habitableRooms:habitable,totalCircuits:totalCircuits,generalCircuits:generalCircuits,generalCircuitCodeMinimum:codeGeneralCircuits,generalCircuitGroupingCount:groupingGeneralCircuits,generalBranchCircuitLoadVA:branchCircuitLoadVA,panelSpaces120V:totalCircuits,totalCableFt:totalCable,generalLoadVA:generalLoadVA,smallApplianceVA:smallApplianceVA,laundryVA:laundryVA,baseConnectedVA:generalLoadVA+smallApplianceVA+laundryVA},circuits:circuits,bom:rows,violations:violations,status:violations.length?'NON-COMPLIANT':(unresolvedLayout?'NO KNOWN CONFLICTS / LAYOUT REQUIRED':'NO KNOWN CONFLICTS / VERIFY'),serviceLoadNote:serviceNote,warnings:['NEC 2026 §120.13 branch-circuit load uses 3 VA/ft² for the dwelling general branch-circuit minimum. This is separate from the 2 VA/ft² dwelling service/feeder load calculation.','General-room receptacle device minimum cannot be calculated from room count or square footage alone. Enter the complete wall-space segment list (feet) separated by commas; malformed or negative tokens fail closed.','Bathroom/powder receptacle quantity is a design allowance only. NEC 210.52(D) device quantity/location requires actual sink and placement geometry.','Kitchen receptacle quantity remains layout-driven under 210.52(C); kitchen count alone is insufficient for an exact device minimum.','Live cable footage is an estimating routing model, not an NEC minimum. Replace with measured plan takeoff when available.','Receptacle grouping is a Bruno design assumption; it can increase but never reduce the §120.13 general branch-circuit minimum.','AFCI/GFCI, dedicated equipment circuits, major loads, service/feeder sizing, grounding/bonding and local AHJ requirements remain separate verification layers.']};
}
