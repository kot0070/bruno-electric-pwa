...
function ensureSettingsAuthority(){var body=q('#dj-settings-body'),save=q('#djs-customer-save');if(!body||!save)return;var s=settings(),form=q('#djs-service-rate')&&q('#djs-service-rate').closest('.dj-form');if(!form)return;
 ...
 if(!save.dataset.atomicAuthority){save.dataset.atomicAuthority='1';save.addEventListener('click',function(ev){
   ...
   write(SETTINGS_KEY,x);window.dispatchEvent(new CustomEvent('bruno:dispatch-changed'));schedule();alert('Tax & journal settings saved.');
 },true)}
}
...
function patch(){ensureSettingsAuthority();patchLegacyEditor();patchMetrics();patchMiniInvoices();patchPreview();syncPreviewHistory();document.documentElement.setAttribute('data-be-runtime-authority','v1.1')}
...
window.BrunoRuntimeAuthorityV1={version:'1.1.0',schedule:schedule,customerTaxPct:customerTaxPct,reservePct:reservePct};
