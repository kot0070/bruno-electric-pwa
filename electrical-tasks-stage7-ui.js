...
var COPY={
 ...
 HVAC_CIRCUIT:{button:'Calculate HVAC Circuit',select:'HVAC Circuit selected. Enter equipment-derived design current and OCPD explicitly; MCA/MOCP and equipment rules are not inferred.',note:'<b>HVAC Circuit:</b> uses the audited sizing chain from explicit equipment-derived inputs. Enter the conductor design current and OCPD from verified equipment/design information. MCA, MOCP, compressor/overload, disconnect, and listing requirements are not derived by this adapter.'},
 MOTOR_CIRCUIT:{button:'Calculate Motor Circuit',select:'Motor Circuit selected. Enter an already-derived motor conductor design current and explicit OCPD; keep Load Basis = NONCONTINUOUS to avoid a second generic 125% factor.',note:'<b>Motor Circuit:</b> reuses the audited conductor/VD/raceway/EGC chain only after motor-specific sizing has already produced the conductor design current. Table FLC, 125% motor conductor sizing, short-circuit/ground-fault protection, overload, controller/disconnect and multi-motor rules are not inferred.'},
 TRANSFORMER_FEED:{button:'Calculate Transformer Feed',select:'Transformer Feed selected. Enter an already-derived primary or secondary conductor design current and explicit OCPD; keep Load Basis = NONCONTINUOUS to avoid a second generic multiplier.',note:'<b>Transformer Feed:</b> reuses the audited sizing chain from explicit already-derived conductor design current. kVA-to-current derivation, primary/secondary protection, tap rules, impedance/fault current, grounding/bonding and secondary-system configuration are not inferred.'}
};
...
root.BrunoElectricalTasksStage7UI=Object.freeze({install:install,decorate:decorate,currentType:currentType,version:'electrical-tasks-stage7-ui-v5'});
