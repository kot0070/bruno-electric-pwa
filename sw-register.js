(function () {
  ...
  function loadPricingDomainGuard(){script('./electric-pricing-domain-guard.js','data-be-pricing-domain-guard','BrunoPricingDomainGuard',true)}
  function loadQuoteLifecycle(){script('./electric-quote-lifecycle.js','data-be-quote-lifecycle','BrunoQuoteLifecycle',true)}
  ...
  function loadAppNavigation(){prepareStrictMarginsRuntime();loadCatalogCostSemantics();loadPricingMarginsSemantics();loadJobMaterialCostSemantics();loadJobSummarySemantics();loadPricingDomainGuard();loadQuoteLifecycle();loadFixedPriceInvoice();loadCustomMaterials();loadCatalogJobUXSemantics();loadCompactHeader();loadDispatchJournal();loadResidentialWorkspaceBridge();loadResidentialHistoryJobScope();loadResidentialWireTakeoff();loadResidentialApplyJob();loadResidentialSaveArchiveUX();if(window.BrunoElectricAppNavigation){loadWorkspaceEnhancement();return}if(document.querySelector('script[data-be-app-nav]'))return;var n=document.createElement('script');n.src='./electric-app-navigation.js';n.defer=true;n.dataset.beAppNav='1';n.onload=loadWorkspaceEnhancement;n.onerror=loadWorkspaceEnhancement;document.head.appendChild(n)}
  prepareStrictMarginsRuntime();
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',loadAppNavigation,{once:true});else loadAppNavigation();
