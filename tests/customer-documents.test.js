...
test('single runtime build source is wired through deterministic bootstrap',function(){ok(reg.indexOf("BUILD_VERSION='1.18'")>=0,'bootstrap build version is not v1.18');ok(version.indexOf("var VERSION='v1.18'")>=0,'visible app version is not v1.18');ok(reg.indexOf("'./electric-app-version.js'")>=0,'version authority is not bootstrapped');ok(index.indexOf('./sw-register.js?v=1.17')>=0,'index still uses a stale bootstrap cache key');ok(index.indexOf("badge.textContent = 'v1.15'")<0,'index still owns a stale visible version writer')});
