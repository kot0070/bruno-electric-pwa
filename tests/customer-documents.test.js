...
test('single runtime build source is wired through deterministic bootstrap',function(){var bm=reg.match(/BUILD_VERSION='([^']+)'/),vm=version.match(/var VERSION='v([^']+)'/);ok(bm,'bootstrap build version missing');ok(vm,'visible app version missing');var build=bm[1],visible=vm[1];ok(build===visible,'bootstrap '+build+' differs from visible '+visible);ok(reg.indexOf("'./electric-app-version.js'")>=0,'version authority is not bootstrapped');ok(index.indexOf('./sw-register.js?v='+build)>=0,'index bootstrap cache key differs from current runtime '+build);ok(index.indexOf("badge.textContent = 'v1.15'")<0,'index still owns a stale visible version writer')});
