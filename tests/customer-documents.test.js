...
test('single runtime build source is wired through deterministic bootstrap',function(){var bm=reg.match(/BUILD_VERSION='([^']+)'/),vm=version.match(/var VERSION='v([^']+)'/);ok(bm,'bootstrap build version missing');ok(vm,'visible app version missing');var build=bm[1],visible=vm[1];ok(build===visible,'bootstrap '+build+' differs from visible '+visible);ok(reg.indexOf("'./electric-app-version.js'")>=0,'version authority is not bootstrapped');ok(/\.\/sw-register\.js\?v=[0-9.]+/.test(index),'index bootstrap is not versioned');ok(index.indexOf("badge.textContent = 'v1.15'")<0,'index still owns a stale visible version writer')});
