...
async function openJournal(page){await page.goto('/index.html',{waitUntil:'load'});await expect(page.locator('#panel-dispatch')).toBeVisible();await expect(page.locator('#dj-add')).toBeVisible();await expect(page.locator('#djs-service-rate')).toBeAttached();await expect(page.locator('#djs-business-reserve')).toBeAttached();}
...
test('JOURNAL-TAX-SEPARATION-01 internal reserve always tracks revenue while customer sales tax is separate',async({page})=>{
  ...
  await page.locator('#djc-save').click();
  await expect.poll(()=>page.evaluate(k=>{const c=JSON.parse(localStorage.getItem(k)).calls[0];return [c.invoiceChargeSalesTax,c.customerSalesTaxPctApplied];},DATA_KEY)).toEqual([true,8.25]);
