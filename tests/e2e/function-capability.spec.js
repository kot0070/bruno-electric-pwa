...
async function fillTaskDesign(page,overrides={}){
  const x=Object.assign({load:'20',voltage:'240',phase:'1',distance:'50',material:'Cu',install:'EMT',conductor:'THHN_THWN2',basis:'NONCONTINUOUS',vd:'3',ambient:'30',ccc:'2',terminal:'75',parallel:'0',maxSize:'',raceway:'SEPARATE_SETS',ocpd:'20',neutral:'NONE',egc:'Cu'},overrides);
...
test('E2E-01 app shell navigation, deep-link and back-forward',async({page},testInfo)=>{
  ...
  await page.goto('/index.html#be=ESTIMATE&tab=catalog',{waitUntil:'load'});
  await expect(page.locator('#panel-catalog')).toBeVisible();
  await page.goBack({waitUntil:'load'});await expect(page.locator('#panel-quote')).toBeVisible();
  await page.goForward({waitUntil:'load'});await expect(page.locator('#panel-catalog')).toBeVisible();
  await page.goBack({waitUntil:'load'});await expect(page.locator('#panel-quote')).toBeVisible();
...
test('E2E-02 Job A/B isolation through user import',async({page},testInfo)=>{
  ...
  await seedJob(page);await openStable(page,'/index.html');page.on('dialog',d=>d.accept());
  ...
  expect(await page.evaluate(k=>JSON.parse(localStorage.getItem(k)).materialsUsed.map(x=>x.item),JOB_KEY)).toEqual(['B-only']);
});

// E2E-03 — standard + custom Catalog UI, blank-vs-zero Your Cost, quantity and reload persistence.
test('E2E-03 Catalog standard/custom material and blank/zero Your Cost persist correctly',async({page},testInfo)=>{
  ...
  await seedJob(page,job);await openStable(page,'/index.html#be=ESTIMATE&tab=catalog');
  await page.locator('#cat-search').fill('20A 1-pole breaker');
  const standardRow=page.locator('#cat-groups tr').filter({hasText:'20A 1-pole breaker'}).first();
  await expect(standardRow).toBeVisible();await standardRow.locator('.cat-add').click();
  ...
  let reload=page.waitForEvent('load');
  ...
  expect(result.standard).toBe(true);expect(result.unresolved).toBe(true);expect(result.zero).toEqual({unitCost:0,costState:'RESOLVED'});
});
...
test('E2E-04 Residential calculate, save, reload, load, apply and history',async({page},testInfo)=>{
  test.skip(desktopOnly(testInfo),'Desktop core journey.');
  await seedJob(page);await openStable(page,'/electrical-tools.html');await openTool(page,'res-live');
  const facts=page.locator('#tool-res-live details').filter({has:page.locator('#rl-sqft')});await facts.locator('summary').click();
  await expect(page.locator('#rl-sqft')).toBeVisible();
  await page.locator('#rl-sqft').fill('1800');await page.locator('#rl-bed').fill('3');await page.locator('#rl-bath').fill('2');await page.locator('#rl-kitchen').fill('1');await page.locator('#rl-laundry').fill('1');await page.locator('#rl-sqft').dispatchEvent('input');
  await expect(page.locator('#rl-out')).not.toContainText('Live takeoff error');page.on('dialog',d=>d.accept());
  ...
  await page.locator('#rl-save-ux-btn').click();await expect(page.locator('#rl-save-state')).toContainText('SAVED · NOT APPLIED');
  expect(await page.evaluate(k=>{const j=JSON.parse(localStorage.getItem(k));return[(j.materialsUsed||[]).length,(j.materialsUnresolved||[]).length];},JOB_KEY)).toEqual(before);
  await page.reload({waitUntil:'load'});await openTool(page,'res-live');
  await page.locator('#rl-save-archive-ux details > summary').click();await expect(page.locator('#rl-archive-overview')).toContainText('1800');
  const loadButton=page.locator('#rl-archive-overview [data-ux-load]').first();await expect(loadButton).toBeVisible();await loadButton.click();await expect(page.locator('#rl-sqft')).toHaveValue('1800');
  await page.locator('#rl-save-ux-btn').click();await expect(page.locator('#rl-apply-job-btn')).toBeEnabled();await page.locator('#rl-apply-job-btn').click();await expect(page.locator('#rl-save-state')).toContainText('APPLIED TO JOB');
  const applied=await page.evaluate(k=>{const j=JSON.parse(localStorage.getItem(k));return(j.materialsUsed||[]).length+(j.materialsUnresolved||[]).length;},JOB_KEY);expect(applied).toBeGreaterThan(before[0]+before[1]);
...
test('E2E-05 Electrical Tasks Feeder calculate save reload apply edit update history',async({page},testInfo)=>{
  test.skip(desktopOnly(testInfo),'Desktop core journey.');test.setTimeout(60000);
  await seedJob(page);await openStable(page,'/electrical-tools.html');await openTool(page,'tasks');
  await page.locator('#et-name').fill('E2E Feeder');await fillTaskDesign(page);await page.locator('#et-calculate').click();
  await expect(page.locator('#et-calc-out')).toContainText('PASS');await expect(page.locator('#et-grounding-out')).toContainText('STAGE 4 PASS');
  const before=await page.evaluate(k=>{const j=JSON.parse(localStorage.getItem(k));return(j.materialsUsed||[]).length+(j.materialsUnresolved||[]).length;},JOB_KEY);
  await page.locator('#et-save').click();await expect(page.locator('#et-status')).toContainText('saved');await expect(page.locator('#et-stage6-status')).toContainText('SAVED');
  ...
  await page.reload({waitUntil:'load'});await openTool(page,'tasks');await expect(page.locator('#et-name')).toHaveValue('E2E Feeder');await openTaskAdvanced(page);
  await page.locator('#et-calculate').click();await expect(page.locator('#et-grounding-out')).toContainText('STAGE 4 PASS');await page.locator('#et-build-takeoff').click();await expect(page.locator('#et-takeoff-out')).toContainText('PASS');
  await expect(page.locator('#et-apply-materials')).toBeEnabled();await page.locator('#et-apply-materials').click();await expect(page.locator('#et-takeoff-status')).toContainText('Applied');await expect(page.locator('#et-stage6-status')).toContainText('APPLIED TO JOB',{timeout:5000});
  const appliedRevision=await page.evaluate(k=>{const j=JSON.parse(localStorage.getItem(k));const a=(j.electricalTaskMaterialApplications||[]).slice(-1)[0];return a&&a.sourceTaskRevision;},JOB_KEY);expect(Number(appliedRevision)).toBeGreaterThan(0);
  await page.locator('#et-distance').fill('40');await page.locator('#et-save').click();await expect(page.locator('#et-status')).toContainText('saved');await expect(page.locator('#et-stage6-status')).toContainText('CHANGED SINCE APPLY',{timeout:5000});
  const oldRows=await page.evaluate(k=>{const j=JSON.parse(localStorage.getItem(k));return(j.materialsUsed||[]).concat(j.materialsUnresolved||[]).filter(x=>x.materialType==='ELECTRICAL_TASK_TAKEOFF').map(x=>x.sourceTaskRevision);},JOB_KEY);expect(oldRows.length).toBeGreaterThan(0);expect(oldRows.every(x=>Number(x)===Number(appliedRevision))).toBe(true);
  await page.locator('#et-calculate').click();await expect(page.locator('#et-grounding-out')).toContainText('STAGE 4 PASS');await page.locator('#et-stage6-update').click();await expect(page.locator('#et-stage6-message')).toContainText('Job updated from task revision');await expect(page.locator('#et-stage6-message')).toContainText('Archived');await expect(page.locator('#et-stage6-status')).toContainText('APPLIED TO JOB',{timeout:5000});
  const history=await page.evaluate(k=>{const j=JSON.parse(localStorage.getItem(k));const t=(j.electricalTasks||[]).find(x=>x.id===j.electricalTaskActiveId);const rows=(j.materialsUsed||[]).concat(j.materialsUnresolved||[]).filter(x=>x.materialType==='ELECTRICAL_TASK_TAKEOFF');return{history:(j.electricalTaskMaterialHistory||[]).length,revision:t&&t.revision,rowRevisions:rows.map(x=>x.sourceTaskRevision)};},JOB_KEY);expect(history.history).toBeGreaterThan(0);expect(history.rowRevisions.length).toBeGreaterThan(0);expect(history.rowRevisions.every(x=>Number(x)===Number(history.revision))).toBe(true);
});
...
test('E2E-06 advanced Electrical Task templates calculate through rendered UI',async({page},testInfo)=>{
  test.skip(desktopOnly(testInfo),'Desktop core journey.');test.setTimeout(60000);await seedJob(page);await openStable(page,'/electrical-tools.html');await openTool(page,'tasks');
  const sel=page.locator('#et-type');
  const cases=[['BRANCH_CIRCUIT_RUN',{}],['EVSE_CIRCUIT',{basis:'CONTINUOUS'}],['HVAC_CIRCUIT',{load:'32',ocpd:'40'}],['MOTOR_CIRCUIT',{}],['TRANSFORMER_FEED',{load:'125',voltage:'480',phase:'3',ccc:'3',ocpd:'150'}],['GENERATOR_FEEDER',{load:'100',ocpd:'125'}],['LONG_DISTANCE_VD',{load:'30',distance:'1000',parallel:'1',ocpd:'30'}]];
  for(const[type,values]of cases){await sel.selectOption(type);expect(await sel.inputValue()).toBe(type);await fillTaskDesign(page,values);await page.locator('#et-calculate').click();await expect(page.locator('#et-status')).toContainText('calculation completed');if(type==='LONG_DISTANCE_VD')await expect(page.locator('#et-grounding-out')).toContainText(/STAGE 4 PASS|REVIEW REQUIRED|NO SUPPORTED CONFIGURATION/);else await expect(page.locator('#et-grounding-out')).toContainText('STAGE 4 PASS');}
});
...
test('E2E-07 Professional Task Solver no guessing and no auto side effects',async({page},testInfo)=>{
  test.skip(desktopOnly(testInfo),'Desktop core journey.');await seedJob(page);await openStable(page,'/electrical-tools.html');await openTool(page,'tasks');
  await page.locator('#et-solver-text').fill('I need a 100A panel 250 ft from service at 240 V.');await page.locator('#et-solver-extract').click();await expect(page.locator('#et-solver-known')).toContainText(/100|250|240/);await expect(page.locator('#et-solver-unresolved')).not.toBeEmpty();
...
test('E2E-09 representative Job export and import',async({page},testInfo)=>{
  test.skip(desktopOnly(testInfo),'Desktop core journey.');await seedJob(page,baseJob('Exported Customer'));await openStable(page,'/index.html');await page.locator('#btn-compact-more').click();await expect(page.locator('#btn-export')).toBeVisible();
  const downloadPromise=page.waitForEvent('download');await page.locator('#btn-export').click();const download=await downloadPromise;expect(download.suggestedFilename()).toMatch(/bruno.*\.json/i);page.on('dialog',d=>d.accept());
  const changed=baseJob('Imported Customer'),env={brunoExportType:'job',brunoExportVersion:1,appVersion:'e2e',exportedAt:new Date().toISOString(),payload:changed};await page.locator('#btn-import').setInputFiles({name:'import.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(env))});await expect.poll(()=>page.evaluate(k=>JSON.parse(localStorage.getItem(k)).quote.customer,JOB_KEY)).toBe('Imported Customer');
});
...
test('E2E-10 reload and stale identifier resilience',async({page},testInfo)=>{
  test.skip(desktopOnly(testInfo),'Desktop core journey.');const j=baseJob('Stale Safe');j.electricalTaskActiveId='missing-task-id';j.electricalTasks=[];j.materialsUnresolved=[{item:'Legacy unresolved',qty:1,yourCost:''}];await seedJob(page,j);await openStable(page,'/index.html');await page.reload({waitUntil:'load'});await expect(page.locator('body')).toBeVisible();await page.goto('/electrical-tools.html',{waitUntil:'load'});await openTool(page,'tasks');await expect(page.locator('#et-status')).not.toContainText(/TypeError|ReferenceError/);
});
...
test('E2E-11 critical actions remain reachable at desktop, phone and tablet',async({page},testInfo)=>{
  await seedJob(page);await openStable(page,'/index.html');await expect(page.locator('#btn-compact-more')).toBeVisible();await page.locator('#btn-compact-more').click();await expect(page.locator('#btn-export')).toBeVisible();await expect(page.locator('#btn-blank')).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth-document.documentElement.clientWidth)).toBeLessThanOrEqual(2);
  await page.goto('/electrical-tools.html',{waitUntil:'load'});await expect(page.locator('#tool-host')).toBeVisible();await openTool(page,'tasks');await expect(page.locator('#et-save')).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth-document.documentElement.clientWidth)).toBeLessThanOrEqual(2);expect(['chromium-desktop','chromium-phone','chromium-tablet']).toContain(testInfo.project.name);
});
