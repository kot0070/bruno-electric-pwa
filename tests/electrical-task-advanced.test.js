...
function throws(fn){var hit=false;try{fn()}catch(e){hit=true}ok(hit,'expected throw')}
...
test('Branch Circuit missing compliance-critical current fails closed',function(){var r=A.calculate('BRANCH_CIRCUIT_RUN',input({loadAmps:''}),ground());ok(r.status!=='PASS','blank design current must not pass')});
test('Branch Circuit blank OCPD/neutral facts remain fail-closed in grounding chain',function(){var r=A.calculate('BRANCH_CIRCUIT_RUN',input(),ground({ocpdAmps:'',neutralMode:'UNRESOLVED'}));ok(r.status!=='PASS','unresolved grounding facts must not pass')});
