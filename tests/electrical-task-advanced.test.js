...
test('Stage 7C HVAC reuses deterministic shared calculation chain from explicit equipment facts',function(){var r=A.calculate('HVAC_CIRCUIT',input({loadAmps:'32',loadBasis:'NONCONTINUOUS'}),ground({ocpdAmps:'40'}));is(r.status,'PASS');ok(r.result,'HVAC result missing');ok((r.warnings||[]).some(function(x){return x.indexOf('HVAC Circuit treats Load / Design Current as an explicit equipment-derived conductor design current')>=0}),'HVAC scope warning missing');is(r.taskTemplateEngineVersion,'electrical-task-advanced-v3')});
test('HVAC missing equipment-derived design current fails closed',function(){throws(function(){A.calculate('HVAC_CIRCUIT',input({loadAmps:''}),ground({ocpdAmps:'40'}))},'HVAC blank design current must fail closed')});
test('HVAC missing explicit OCPD fact fails closed',function(){throws(function(){A.calculate('HVAC_CIRCUIT',input({loadAmps:'32'}),ground({ocpdAmps:''}))},'HVAC blank OCPD must fail closed')});
test('Stage 7C adapter refuses later templates',function(){['MOTOR_CIRCUIT','TRANSFORMER_FEED','GENERATOR_FEEDER'].forEach(function(t){throws(function(){A.calculate(t,input(),ground())},t+' should remain locked')})});
