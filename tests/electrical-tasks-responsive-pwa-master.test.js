...
if(typeof require==='function'){
 ...
 test('Stage 11 PWA cache advances and retains offline Electrical Tasks shell',function(){ok(sw.indexOf("const CACHE = 'bruno-electric-v67'")>=0,'Stage 11 cache version missing');['./electrical-tools.html','./electric-electrical-tasks.js','./electric-electrical-task-solver.js','./electrical-tasks-stage8-ui.js'].forEach(function(x){ok(sw.indexOf("'"+x+"'")>=0,x+' missing from offline core shell')});ok(sw.indexOf('OWNED_CACHE_RE')>=0,'owned cache guard missing');ok(sw.indexOf("caches.match('./index.html')")>=0,'navigation offline fallback missing')});
}
