...
test('invoice path is explicitly separate from T&M print controls',function(){ok(src.indexOf("['btn-print-tm','Print T&M Invoice']")>=0,'header T&M relabel missing');ok(src.indexOf("['btn-print-tm-2','Print T&M Invoice (PDF)']")>=0,'panel T&M relabel missing');ok(src.indexOf("b.id='qa-print-fixed'")>=0,'fixed invoice button missing');ok(src.indexOf("b.textContent='Print Fixed-Price Invoice'")>=0,'fixed invoice action label missing')});
