...
test('shared IA defines same five sections for all shells',function(){['Job','Estimate','Electrical','Billing','More'].forEach(function(label){ok(appNav.indexOf("label:'"+label+"'")>=0,label+' missing')});ok(workspace.indexOf('groups=NAV&&NAV.groups')>=0,'workspace does not consume canonical IA');ok(shell.indexOf('groups=NAV&&NAV.groups')>=0,'Electrical shell does not consume canonical IA')});
