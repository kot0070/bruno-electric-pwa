...
test('unconfirmed garage suppresses extra garage material even if UI default supplies one extra',function(){var r=T.calculate({kitchens:0,garageBays:0,garageExtraReceptacles:1});var rec=findBom(r,'20A duplex receptacle'),box=findBom(r,'1-gang new-work device box'),plate=findBom(r,'1-gang device wall plate allowance');ok(!r.circuits.some(function(x){return x.label==='Garage receptacles'}));ok(!rec||rec.qty===0,'phantom garage receptacle');ok(!box||box.qty===0,'phantom garage box');ok(!plate||plate.qty===0,'phantom garage plate')});
...
test('outdoor devices do not also create indoor new-work boxes',function(){var r=T.calculate({kitchens:0,outdoorGfci:2});var box=findBom(r,'1-gang new-work device box');ok(!box||box.qty===0,'outdoor devices double-counted as indoor new-work boxes')});
